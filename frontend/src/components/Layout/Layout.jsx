...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  // Get icon for a page based on its path or name
  const getPageIcon = (page) => {
    const path = formatUrl(page.url).toLowerCase();
    const name = page.name?.toLowerCase() || '';
    
    // Map paths and names to icons
    ...
    // Default to Globe for other internal pages
    return Globe;
  };
              ...
              <ul className={`space-y-1 overflow-y-auto ${myPages.length > 10 ? 'max-h-[400px]' : 'max-h-none'} scrollbar-hide`}>
                {myPages
                  ...
                  .map((p, index) => {
                    ...
                    const pageKey = p.id ?? `assigned-page-${p.name || 'untitled'}-${p.url || ''}-${index}`;
                    const isExpanded = !!expandedMenus[pageKey];
                    const PageIcon = isExternal ? ExternalLink : getPageIcon(p);
                        ...
                        <div className="flex items-center">
                          ...
                          {isExternal ? (
                            ...
                            <Link
                              to={href}
                              onClick={onClose}
                              className={`flex items-center gap-3 flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${pageActive
                        ...
                        {hasChildren && isExpanded && (
                          <ul className="ml-8 mt-1 space-y-1 border-l-2 border-gray-200 dark:border-gray-700 pl-2">
                            {p.children.map((child, childIndex) => {
                              ...
                              const childKey = child.id ?? `${pageKey}-child-${child.name || 'untitled'}-${child.url || ''}-${childIndex}`;
                                ...
                                <li key={childKey}>
                                  {childExternal ? (
                                    ...
                                    <Link
                                      to={childHref}
                                      onClick={onClose}
                                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 ${childActive
